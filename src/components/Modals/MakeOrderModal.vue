<template>
    ...
    <template v-if="!showAuthModal">
      ...
      >
        <div class="modal__body">
          <div class="modal__title" :class="{ 'empty': !getBasket.length }">
            Корзина {{ getBasket.length ? `(${getBasket.length})` : 'пуста' }}
          </div>

          <div class="modal__content modal__content_mt-30" v-if="getBasket.length">
            <MakeOrderTable
                :products="getBasket"
                @set-count-of-product="setCountOfProduct"
                @delete-product="deleteProduct"
                @open-login-modal="openAuthModal"
  ...
  <LoginModal @close="showAuthModal = false" v-if="showAuthModal"/>
</template>
...
export default {
  name: "MakeOrderModal",
  components: {LoginModal, MakeOrderTable},
  emits: ['close'],
  data() {
    return {
      modal: {
  ...
  methods: {
    ...
    setCountOfProduct(amount, productId) {
      if (productId) {
        const index = this.basket.map(item => item.id).indexOf(productId)
  ...
  mounted() {
    this.$store.commit('products/SET_PRODUCT_FOR_FAST_BUY', null)
...
@import "@/assets/scss/modals/_makeOrderModal.scss";
